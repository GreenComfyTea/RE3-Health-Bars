...
-- local do_awake_method = enemy_controller_type_def:get_method("doAwake");
local get_no_damage_method = enemy_controller_type_def:get_method("get_NoDamage");
local apply_revive_method = enemy_controller_type_def:get_method("ApplyRevive");
local do_start_b_646_1_method = enemy_controller_type_def:get_method("<doStart>b__646_1");
...
function this.on_revive(enemy_controller)
	local enemy = this.get_enemy(enemy_controller);
	if enemy == nil then
		return;
	end
...
function this.init_module()
	...
	-- sdk.hook(apply_revive_method, function(args)
	-- 	local enemy_controller = sdk.to_managed_object(args[2]);
	-- 	this.on_revive(enemy_controller);

	-- end, function(retval)
	-- 	return retval;
	-- end);

	sdk.hook(do_start_b_646_1_method, function(args)
		local enemy_controller = sdk.to_managed_object(args[2]);
		this.on_revive(enemy_controller);
